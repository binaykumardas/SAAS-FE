 ...
 * @description FIXED: Added Null-checks and Optional Chaining to prevent blank screen crashes.
 */
...
interface ProfileSidebarProps {
  // We make 'basic' optional here (?) so it doesn't crash if Profile.tsx passes null
  basic?:      BasicDetails; 
...
const ProfileSidebar = ({
  ...
  // ── BUG FIX: Fallback Name ──────────────────────────────────
  // If firstName and lastName are missing, show "User Name" instead of crashing
    ...
    <div className="lg:col-span-1 flex flex-col gap-4">
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-card">
        ...
        <div className="relative w-fit mx-auto mb-5">
          <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-accent ring-4 ring-accent-tint">
            <div className="w-full h-full bg-raised flex items-center justify-center">
              <svg width="40" height="40" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2} className="text-muted">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
              </svg>
            </div>
          </div>

          <button className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-accent hover:bg-accent-hover text-white border-2 border-surface flex items-center justify-center transition-all duration-150 shadow-accent">
        ...
        <div className="text-center mb-4">
          {/* BUG FIX: Using displayName variable which handles null/undefined */}
        ...
        <div>
          ...
          <p className="text-sm text-secondary leading-relaxed">
            {/* BUG FIX: Added optional chaining and fallback text */}
            {basic?.aboutMe || "No bio added yet."}
          </p>
        </div>
      </div>
